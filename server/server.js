...
// API to get all verified users
app.get("/api/users/verified", (req, res) => {
  const query = "SELECT * FROM Users WHERE verified = 'YES' ORDER BY created_at DESC";
  db.query(query, (err, results) => {
    if (err) return res.status(500).json({ message: "Server error." });
    res.status(200).json(results);
  });
...
// API to deactivate a verified user (set verified to 'NO')
app.post("/api/users/deactivate/:userId", (req, res) => {
  const { userId } = req.params;
  ...
  const query = "UPDATE Users SET verified = 'NO' WHERE user_id = ?";
  db.query(query, [userId], (err, result) => {
    if (err) return res.status(500).json({ message: "Server error." });
